...
void AdresatMenedzer::usunWybranegoAdresataZPliku(int idUsuwanegoAdresata)
{
    ...
    if (odczytywanyPlikTekstowy.good() == true && idUsuwanegoAdresata != 0)
    {
        ...
        MetodyPomocnicze::usunPlik(nazwaPlikuZAdresatami);
        MetodyPomocnicze::zmienNazwePliku(nazwaTymczasowegoPlikuZAdresatami, nazwaPlikuZAdresatami);
    }
}

void AdresatMenedzer::edytujAdresataWPliku(int numerEdytowanejLinii, string liniaZDanymiAdresataOddzielonePionowymiKreskami)
{
    fstream odczytywanyPlikTekstowy, tymczasowyPlikTekstowy;
    string wczytanaLinia = "";
    int numerWczytanejLinii = 1;
